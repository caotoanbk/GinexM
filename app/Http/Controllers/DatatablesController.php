...
class DatatablesController extends Controller
{
	...
	{
		$yclhangs = Dntung::select(['id', 'user_id', 'stokhai', 'slcont', 'slchroi', 'slcnong', 'slclanh', 'hangtau', 'nhaxe', 'created_at', 'bill', 'slc20','slc40', 'khang', 'lcont', 'check', 'approve', 'done', 'ndonghang', 'nyeucau', 'ngiaohang', 'nnhanhang', 'khachhang', 'tuyenduong', 'loaihang', 'curator_check'])->where('done', false)->where('user_id', \Auth::user()->id);
		return Datatables::of($yclhangs)->addColumn('filebooking', function($yclhang){
			...
			if($check && $approve) return '<small class="text-success"><em><b>Được phép tạm ứng</b></em></small>';
		})->addColumn('check', function($yclhang){return '';})->addColumn('resp', function($yclhang){return '';})->make(true);
	}
	public function dntung_home()
	{
		$yclhangs = Dntung::select(['id', 'user_id', 'created_at', 'reason', 'bill', 'slc20','slc40', 'khang', 'ttien', 'lcont', 'tghung', 'cuoc', 'nang', 'ha', 'hquan', 'psinh', 'check', 'approve', 'done', 'ndonghang', 'ttien_ltron', 'nyeucau', 'ngiaohang', 'nnhanhang', 'khachhang', 'tuyenduong', 'loaihang', 'curator_check'])->where('approve', true)->where('curator_check', true)->where('done', false)->where('user_id', \Auth::user()->id);
		return Datatables::of($yclhangs)->addColumn('filebooking', function($yclhang){
			return '<a href="/de_nghi_tam_ung/'.$yclhang->id.'.pdf'.'". class="btn btn-xs btn-primary">View</a>';	
		})->addColumn('status', function($yclhang){
			$ttien = $yclhang->ttien;
			if($ttien > 0)
				return '<small class="text-success"><em><b>Đã yêu cầu tạm ứng</b></em></small>';
			else
				return '<small class="text-warning"><em><b>Chưa yêu cầu tạm ứng</b></em></small>';
			//$check=$yclhang->check;
			//if($check){
	...
	{
		$yclhangs = Dntung::select(['id', 'user_id', 'created_at', 'reason', 'bill', 'slc20','slc40', 'khang', 'ttien', 'lcont', 'tghung', 'cuoc', 'nang', 'ha', 'hquan', 'psinh', 'check', 'approve', 'done', 'ndonghang', 'ttien_ltron', 'nyeucau', 'ngiaohang', 'nnhanhang', 'loaihang', 'khachhang', 'tuyenduong', 'curator_check'])->where('dntungs.curator_check', true)->where('approve', true)->where('done', false)->where('user_id', '=', \Auth::user()->id)->where('ndonghang', '>=', Carbon::today());
		return Datatables::of($yclhangs)->addColumn('filebooking', function($yclhang){
			return '<a href="/de_nghi_tam_ung/'.$yclhang->id.'.pdf'.'". class="btn btn-xs btn-primary">View</a>';	
		})->addColumn('status', function($yclhang){
	...
	public function sec_tuchthanh_home()
	{
		...
		return Datatables::of($yclhangs)->addColumn('filebooking', function($yclhang){
			...
			$ttien = $yclhang->ttien;
			if($ttien > 0)
				return '<em><small class="text-success"><b>Đã yêu cầu tạm ứng</b></small></em>';
			else
	...
	public function direc_dntung_home()
	{
		...
		return Datatables::of($yclhangs)->addColumn('filebooking', function($yclhang){
			...
			if($ttien > 0){
				...
				return '<em><small class="text-warning"><b>Chưa yêu cầu tạm ứng</b></small></em>';
			}
		})->addColumn('check', function($yclhang){return '';})->addColumn('resp', function($yclhang){return '';})->make(true);
	}
	...
	{
		$yclhangs = Dntung::join('users', 'dntungs.user_id', '=', 'users.id')->select(['dntungs.id', 'dntungs.user_id', 'dntungs.created_at', 'dntungs.reason', 'dntungs.bill', 'dntungs.slc20','dntungs.slc40', 'dntungs.khang', 'dntungs.ttien', 'dntungs.lcont', 'dntungs.tghung', 'dntungs.cuoc', 'dntungs.nang', 'dntungs.ndonghang', 'dntungs.ha', 'dntungs.hquan', 'dntungs.psinh', 'dntungs.check', 'dntungs.approve', 'dntungs.done', 'users.name', 'dntungs.ttien_ltron', 'dntungs.nyeucau', 'dntungs.ngiaohang', 'dntungs.nnhanhang', 'dntungs.loaihang', 'dntungs.khachhang', 'dntungs.tuyenduong', 'dntungs.curator_check'])->where('dntungs.curator_check', true)->where('approve', true)->where('dntungs.ttien', '>', 0)->where('done', false)->where('dntungs.ndonghang', '>=', Carbon::today());
		return Datatables::of($yclhangs)->addColumn('filebooking', function($yclhang){
			return '<a href="/de_nghi_tam_ung/'.$yclhang->id.'.pdf'.'". class="btn btn-xs btn-primary">View</a>';	
		})->addColumn('status', function($yclhang){
			$ngaydonghang= new Carbon($yclhang->ndonghang);
